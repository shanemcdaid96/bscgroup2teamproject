...
$result = mysqli_query($connection, "SELECT * FROM emails Where SubjectID=".$_GET['SubjectID']."");
 ...
 print('<p style="font-size: 17px; display:inline;"><i>From: </i></p><p style="font-size: 17px;display:inline; ">');
  print($row2['username']);
    print('</p></br>');
 print('<p style="font-size: 17px; display:inline;"><i>Subject: </i></p><p style="font-size: 17px; display:inline;">');
  ...
  print($row['Message']);
    print('</p><hr>');
  print('<a href="compose.php?ReceiverID='.$row['SenderID'].'"><img src=../reply.png width=105 height=35></a>');
  print('&nbsp;&nbsp;&nbsp;&nbsp;');
  print('<a href="../uploads/'.$rowAttachmentID['Filename'].'"download><img src=../att.png width=55 height=55></a>');
    print('&nbsp;&nbsp;&nbsp;&nbsp;');

  print('<a href="delete.php?id='.$row['SubjectID'].'"><img src=../deleteEmail.png width=45 height=45></a><hr>');

    //    $resultAttachmentID=mysqli_query($connection,"SELECT * FROM attachments WHERE IDemail=".$row['SubjectID']."");
    // $rowAttachmentID=mysqli_fetch_assoc($resultAttachmentID);
    //   print('<td>');
    //   print('<p>Attachment</p>');
    //   print('<img src=uploads/'.$rowAttachmentID['Filename'].' width=50%>');
    print('</td>');
  }
